...
export const queries = {
  ...
  status: (id: string) =>
    queryOptions({ queryKey: ["assessment", id, "status"], queryFn: () => getApi().getStatus(id) }),
  profileForm: () =>
    queryOptions({
      queryKey: ["methodology", "profile-form"],
      queryFn: () => getApi().getProfileForm(),
      staleTime: Infinity,
    }),
  profile: (id: string) =>
    queryOptions({
      queryKey: ["assessment", id, "profile"],
      queryFn: () => getApi().getProfile(id),
    }),
  screening: (id: string) =>
    queryOptions({
      queryKey: ["assessment", id, "screening"],
      queryFn: () => getApi().getScreening(id),
    }),
  nextPrompt: (id: string, domain: DomainCode, after: string | null) =>
    queryOptions({
      queryKey: ["assessment", id, "domain", domain, "next", after ?? "first"],
      queryFn: () => getApi().getNextPrompt(id, domain, after),
    }),
  promptById: (id: string, domain: DomainCode, promptId: string) =>
    queryOptions({
      queryKey: ["assessment", id, "domain", domain, "prompt", promptId],
      queryFn: () => getApi().getPromptById(id, domain, promptId),
    }),
  evidence: (id: string) =>
    queryOptions({
      queryKey: ["assessment", id, "evidence"],
      queryFn: () => getApi().getEvidence(id),
    }),
  results: (id: string) =>
    queryOptions({
      queryKey: ["assessment", id, "results"],
      queryFn: () => getApi().getPreliminaryResults(id),
    }),

  assessorQueue: () =>
    queryOptions({ queryKey: ["assessor", "queue"], queryFn: () => getApi().getAssessorQueue() }),
  assessorAssessment: (id: string) =>
    queryOptions({
      queryKey: ["assessor", id, "overview"],
      queryFn: () => getApi().getAssessorAssessment(id),
    }),
  responseReview: (id: string) =>
    queryOptions({
      queryKey: ["assessor", id, "responses"],
      queryFn: () => getApi().getResponseReview(id),
    }),
  evidenceReview: (id: string) =>
    queryOptions({
      queryKey: ["assessor", id, "evidence"],
      queryFn: () => getApi().getEvidenceReview(id),
    }),
  metricScoring: (id: string) =>
    queryOptions({
      queryKey: ["assessor", id, "metrics"],
      queryFn: () => getApi().getMetricScoring(id),
    }),
  context: (id: string) =>
    queryOptions({
      queryKey: ["assessor", id, "context"],
      queryFn: () => getApi().getContextCalibration(id),
    }),
  scoreRuns: (id: string) =>
    queryOptions({ queryKey: ["assessor", id, "runs"], queryFn: () => getApi().getScoreRuns(id) }),
  executionLog: (id: string) =>
    queryOptions({
      queryKey: ["assessor", id, "log"],
      queryFn: () => getApi().getExecutionLog(id),
    }),
};
...
export function useSaveProfile(assessmentId: string) {
  ...
  return useMutation({
    ...
    onSuccess: () =>
      invalidate(["assessment", assessmentId, "profile"], ["assessment", assessmentId, "status"]),
  });
}
...
export function useCreateEvidence(assessmentId: string) {
  ...
  return useMutation({
    ...
    onSuccess: () =>
      invalidate(["assessment", assessmentId, "evidence"], ["assessment", assessmentId, "status"]),
  });
}
...
export function useSaveMetricScoring(assessmentId: string) {
  const invalidate = useInvalidate();
  return useMutation({
    mutationFn: ({ metricId, input }: { metricId: string; input: MetricScoringInput }) =>
      getApi().saveMetricScoring(assessmentId, metricId, input),
