...
export const Route = createFileRoute("/_workspace/pulse")({
  loader: ({ context }) => context.queryClient.ensureQueryData(queries.screening(context.assessmentId)).then(() => undefined),
  pendingComponent: () => <PagePending />,
  head: () => ({
    meta: [
      { title: "Institutional Pulse — AI Resilient University" },
      { name: "description", content: "A short set of strategic signals that give the assessment an early read of how AI is positioned, governed and practised across the institution." },
      { property: "og:title", content: "Institutional Pulse — AI Resilient University" },
      { property: "og:description", content: "A short strategic diagnostic of institutional signals." },
    ],
  }),
  component: PulsePage,
});
      ...
      <PageContainer width="narrow">
        <div className="mx-auto max-w-2xl py-10 text-center">
          ...
          <p className="mx-auto mt-4 max-w-lg text-[15px] leading-relaxed text-muted-foreground">
            The assessment now has an early sense of how AI sits within the institution. The next stage explores each domain in turn, beginning with D01 · {domainNames.D01}.
          </p>
          ...
          <div className="mt-10 flex flex-wrap items-center justify-center gap-3">
            <Button variant="outline" onClick={() => { setComplete(false); setIndex(0); }}>
              Review signals
            </Button>
  ...
  return (
    <PageContainer width="narrow">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <SignalProgress label={screening.title} caption={`${captured} of ${prompts.length} signals captured`} current={captured} total={prompts.length} />
        <SaveIndicator state={save.isPending ? "saving" : "saved"} />
      </div>
      {index === 0 && captured === 0 && <p className="mt-6 max-w-2xl text-[15px] leading-relaxed text-muted-foreground">{screening.intro}</p>}
      ...
      <div className="mt-8 space-y-6">
        <QuestionCard
          ...
          footer={
            <>
              <Button variant="ghost" className="text-muted-foreground" onClick={() => setIndex((i) => Math.max(0, i - 1))} disabled={index === 0}>
                <ArrowLeft /> Previous signal
              </Button>
              <div className="flex items-center gap-3">
                {!existing && <span className="text-xs text-muted-foreground">Select a response to continue</span>}
        ...
        >
          <ResponsePanel
            ...
            autosaveChoices
          />
          <ResponseControls prompt={prompt} existing={existing} onSave={(input) => save.mutate({ promptId: prompt.id, ...input })} className="mt-3" />
        </QuestionCard>
      ...
      <ol className="mt-10 flex flex-wrap gap-x-6 gap-y-2 border-t border-border pt-6 text-xs text-muted-foreground">
        {prompts.map((p, i) => (
          <li key={p.id}>
            <button type="button" onClick={() => setIndex(i)} className={cn("inline-flex cursor-pointer items-center gap-2 underline-offset-4 hover:underline", i === index && "text-foreground")}>
              <span className={cn("size-1.5 rounded-full", responded.has(p.id) ? "bg-navy" : i === index ? "bg-blue" : "bg-border")} />
              {p.theme}
            </button>
          </li>
        ))}
