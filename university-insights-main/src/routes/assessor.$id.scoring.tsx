...
export const Route = createFileRoute("/assessor/$id/scoring")({
  loader: ({ context, params }) =>
    context.queryClient.ensureQueryData(queries.metricScoring(params.id)).then(() => undefined),
  ...
  head: () => ({
    meta: [
      { title: "Metric scoring — ARUI Assessor" },
      {
        name: "description",
        content:
          "Record maturity, implementation depth and outcome per metric, with N/A decisions, rationale and flags.",
      },
  ...
  return (
    <PageContainer width="wide">
      <PageHeader
        ...
        meta={
          <SegmentedControl
            options={inScopeDomains.map((d) => ({ value: d, label: d }))}
            value={domain}
            onChange={(v) => {
              setDomain(v);
              setSelectedId(null);
              ...
              {list.map((m) => {
                ...
                return (
                  <tr
                    ...
                    className={cn(
                      "cursor-pointer hover:bg-ivory-deep/40",
                      active && "bg-navy/[0.04]",
                    )}
                  >
                    ...
                    <td className="px-3 py-3 text-center font-mono text-xs">
                      {m.outcomeNotApplicable ? (
                        <span className="text-muted-foreground">N/A</span>
                      ) : (
                        (m.outcome ?? <span className="text-muted-foreground">·</span>)
                      )}
                    </td>
...
function MetricEditor({ assessmentId, metric }: { assessmentId: string; metric: MetricScoring }) {
  ...
  const v = { ...metric, ...form };
  const set = <K extends keyof MetricScoringInput>(k: K, val: MetricScoringInput[K]) =>
    setForm((f) => ({ ...f, [k]: val }));
  ...
  return (
    <Panel className="xl:sticky xl:top-24 xl:self-start">
      <div className="border-b border-border px-6 py-4">
        ...
        <h2 className="mt-1 font-sans text-[15px] font-semibold tracking-normal text-foreground">
          {metric.metricName}
        </h2>
      ...
      <div className="space-y-6 px-6 py-6">
        <Field
          label="Applicable to this institution"
          hint="N/A requires genuine contextual inapplicability and a documented rationale. Missing evidence or data is not N/A."
        >
          <SegmentedControl
            options={[
        ...
        {v.applicable === false && (
          <Field label="N/A rationale">
            <TextArea
              rows={3}
              value={v.notApplicableRationale ?? ""}
              onChange={(e) => set("notApplicableRationale", e.target.value)}
              placeholder="Why this metric cannot apply to an institution of this type…"
            />
          </Field>
        )}
        ...
        {v.applicable !== false && (
          <>
            <LevelPicker
              label="Maturity (M)"
              value={v.maturity ?? null}
              onChange={(x) => set("maturity", x)}
            />
            <LevelPicker
              label="Implementation depth (I)"
              value={v.implementation ?? null}
              onChange={(x) => set("implementation", x)}
            />
            <div className="space-y-3">
              <LevelPicker
                ...
                onChange={(x) => {
                  set("outcome", x);
                  set("outcomeNotApplicable", false);
                }}
                disabled={!!v.outcomeNotApplicable}
              />
              <label className="flex cursor-pointer items-start gap-2.5 text-[13px] text-muted-foreground">
                <input
                  type="checkbox"
                  className="mt-0.5"
                  checked={!!v.outcomeNotApplicable}
                  onChange={(e) => {
                    set("outcomeNotApplicable", e.target.checked);
                    if (e.target.checked) set("outcome", null);
                  }}
                />
                <span>
                  Outcome legitimately not applicable for this metric{" "}
                  <span className="block text-[11px]">
                    Distinct from a blank outcome. A blank outcome is treated as incomplete, not as
                    N/A.
                  </span>
                </span>
              </label>
              {v.outcomeNotApplicable && (
                <TextArea
                  rows={2}
                  value={v.outcomeNotApplicableRationale ?? ""}
                  onChange={(e) => set("outcomeNotApplicableRationale", e.target.value)}
                  placeholder="Why outcome cannot be observed for this metric…"
                />
              )}
            </div>
            <Field label="Evidence level supporting this score">
              <SelectField
                options={evidenceLevels.map((l) => ({ value: l, label: l }))}
                value={v.evidenceLevel ?? null}
                onChange={(x) => set("evidenceLevel", x as EvidenceLevel)}
                placeholder="Select E0–E4"
              />
            </Field>
          </>
        )}
        ...
        <Field
          label="Rationale"
          hint="Required for every scored or N/A metric. Reference the responses and evidence relied upon."
        >
          <TextArea
            rows={4}
            value={v.rationale ?? ""}
        ...
        {metric.engine && (
          <div className="rounded-md border border-border bg-ivory-deep/40 px-4 py-3 text-xs text-muted-foreground">
            <span className="font-medium text-foreground">Latest run {metric.engine.runId}:</span>{" "}
            metric score {metric.engine.metricScore ?? "—"} · {metric.engine.validation}
          </div>
        )}

        <div className="flex items-center justify-between gap-3 border-t border-border pt-4">
          <span className="text-xs text-muted-foreground">
            {save.isSuccess && !dirty ? "Saved" : dirty ? "Unsaved changes" : ""}
          </span>
          <div className="flex gap-2">
            <Button
              ...
              onClick={() =>
                save.mutate(
                  { metricId: metric.metricId, input: { ...form, status: "draft" } },
                  { onSuccess: () => setForm({}) },
                )
              }
            >
            ...
            <Button
              className="h-9"
              disabled={save.isPending || !(v.rationale ?? "").trim()}
              onClick={() =>
                save.mutate(
                  {
                    ...
                    input: {
                      ...form,
                      status: v.applicable === false ? "not_applicable" : "scored",
                    },
                  },
                  { onSuccess: () => setForm({}) },
                )
...
function LevelPicker({
  ...
  return (
    <Field label={label}>
      <div
        className={cn("grid grid-cols-6 gap-1.5", disabled && "pointer-events-none opacity-40")}
        role="radiogroup"
        aria-label={label}
      >
        {levels.map((l) => (
          <button
            ...
            onClick={() => onChange(l)}
            className={cn(
              "flex h-12 cursor-pointer flex-col items-center justify-center rounded-md border text-center transition-colors",
              value === l
          ...
          >
            ...
            <span
              className={cn(
                "mt-1 text-[9px] uppercase tracking-[0.08em]",
                value === l ? "text-primary-foreground/80" : "text-muted-foreground",
              )}
            >
              {maturityLabels[l]}
            </span>
