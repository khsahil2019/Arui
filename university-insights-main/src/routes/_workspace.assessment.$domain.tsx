...
export const Route = createFileRoute("/_workspace/assessment/$domain")({
  ...
  loader: ({ context, deps }) =>
    context.queryClient.ensureQueryData(deps.p ? queries.promptById(context.assessmentId, context.domain, deps.p) : queries.nextPrompt(context.assessmentId, context.domain, null)).then(() => undefined),
...
function DomainRunner() {
  ...
  const { data } = useSuspenseQuery(p ? queries.promptById(assessmentId, domain, p) : queries.nextPrompt(assessmentId, domain, null));
  ...
  const priorId = data.prompt?.presentation.kind === "matrix" && data.prompt.presentation.rows.source === "prior_response" ? data.prompt.presentation.rows.promptId : null;
  const prior = useQuery({ ...queries.promptById(assessmentId, domain, priorId ?? "none"), enabled: !!priorId });
  const priorRows: ChoiceOption[] | undefined = (() => {
    ...
    const value = prior.data.existingResponse?.state === "answered" ? prior.data.existingResponse.value : null;
    if (pres.kind !== "multi_choice" || !Array.isArray(value)) return [];
    return pres.options.filter((o) => (value as string[]).includes(o.value));
  })();

  const goTo = (promptId: string | undefined) => navigate({ to: ".", search: promptId ? { p: promptId } : {}, params: { domain } });
    ...
    return (
      <PageContainer width="narrow">
        <div className="mx-auto max-w-2xl py-10 text-center">
          ...
          <h1 className="mt-3 text-3xl md:text-4xl">{data.domainComplete ? "This domain is complete for now." : "Nothing further to ask right now."}</h1>
          <p className="mx-auto mt-4 max-w-lg text-[15px] leading-relaxed text-muted-foreground">
            The assessment may return with targeted follow-ups once responses and evidence have been reviewed. You can revisit any response from the list below.
          </p>
          <ol className="mx-auto mt-8 max-w-md divide-y divide-border rounded-lg border border-border bg-card text-left shadow-card">
            {data.history.map((h, i) => (
              <li key={h.promptId}>
                <button type="button" onClick={() => goTo(h.promptId)} className="flex w-full cursor-pointer items-center gap-3 px-4 py-2.5 text-sm hover:bg-ivory-deep/50">
  ...
  const prevId = idx > 0 ? data.history[idx - 1]?.promptId : idx === -1 && data.history.length ? data.history[data.history.length - 1]?.promptId : undefined;
  ...
  return (
    <PageContainer width="narrow">
      <div className="flex flex-wrap items-end justify-between gap-4">
        {position ? (
          <SignalProgress label={`${domain} · ${shortName(domain)}`} caption={`${position.theme} · Exploring ${position.withinTheme.current} of ${position.withinTheme.total}`} current={position.themeIndex} total={position.themeTotal} />
        ) : (
      ...
      {position && (
        <ol className="mt-4 flex flex-wrap gap-x-5 gap-y-1 text-xs text-muted-foreground">
          {position.themes.map((t) => (
            <li key={t.label} className={cn("inline-flex items-center gap-2", t.state === "current" && "text-foreground")}>
              <span className={cn("size-1.5 rounded-full", t.state === "complete" ? "bg-navy" : t.state === "current" ? "bg-blue" : "bg-border")} />
              {t.label}
            </li>
          ))}
        </ol>
      ...
      <QuestionCard
        ...
        theme={prompt.origin === "targeted" ? <StatusBadge tone="amber">Targeted follow-up</StatusBadge> : `Theme ${position ? position.themeIndex + 1 : ""}`}
        question={prompt.prompt}
        help={prompt.help}
        className="mt-6 animate-in fade-in-0 slide-in-from-bottom-1 duration-300"
        footer={
          <>
            <Button variant="ghost" className="text-muted-foreground" disabled={!prevId} onClick={() => goTo(prevId)}>
              <ArrowLeft /> Previous
            </Button>
            <div className="flex items-center gap-3">
              {!hasResponse && <span className="text-xs text-muted-foreground">Save a response, or tell us you're not sure, to continue</span>}
              <Button className="h-10 px-5" disabled={!hasResponse} onClick={() => navigate({ to: ".", params: { domain }, search: {} })}>
                Continue <ArrowRight />
              </Button>
            </div>
          </>
      ...
      >
        ...
        <ResponsePanel prompt={prompt} existing={existing} saving={save.isPending} onSave={(input) => save.mutate({ promptId: prompt.id, ...input })} priorRows={priorRows} autosaveChoices />
        <ResponseControls prompt={prompt} existing={existing} onSave={(input) => save.mutate({ promptId: prompt.id, ...input })} className="mt-4" />
        {prompt.evidenceHints && prompt.evidenceHints.length > 0 && (
          <div className="mt-6 flex flex-wrap items-center gap-2 border-t border-border pt-4 text-xs text-muted-foreground">
            ...
            {prompt.evidenceHints.map((h) => (
              <span key={h} className="rounded-sm border border-border bg-ivory-deep/60 px-2 py-0.5 text-foreground/80">
                {h}
              </span>
            ))}
