...
export const Route = createFileRoute("/assessor/$id/context")({
  loader: ({ context, params }) =>
    context.queryClient.ensureQueryData(queries.context(params.id)).then(() => undefined),
  ...
  head: () => ({
    meta: [
      { title: "Context calibration — ARUI Assessor" },
      {
        name: "description",
        content:
          "Required maturity per domain derived from institutional context, current maturity and diagnostic transformation distance.",
      },
...
function ContextPage() {
  ...
  return (
    <PageContainer width="wide">
      <PageHeader
        eyebrow="Context calibration"
        title="Required maturity and transformation distance"
        lede="Required maturity is derived by the engine from the institution's context profile. Transformation distance is Required − Current and is diagnostic only — it never adjusts a score. Overrides need a reason, evidence and second review."
      />
            ...
            {rows.map((r) => (
              <tr key={r.domainCode} className="align-top">
                ...
                <td className="w-48 px-4 py-4">
                  {r.currentMaturity !== null && r.requiredMaturity !== null ? (
                    <MaturityScale
                      current={r.currentMaturity}
                      required={r.requiredMaturity}
                      className="mt-1.5"
                    />
                  ) : (
                ...
                <td className="px-4 py-4">
                  {r.requiredMaturity !== null
                    ? `${r.requiredMaturity} · ${maturityLabels[r.requiredMaturity]}`
                    : "—"}
                </td>
                <td className="px-4 py-4">
                  {r.currentMaturity !== null
                    ? `${r.currentMaturity} · ${maturityLabels[r.currentMaturity]}`
                    : "—"}
                </td>
                ...
                <td className="px-4 py-4">
                  ...
                  {r.overrideRequested && (
                    <span className="mt-1 block text-[11px] text-amber">Override requested</span>
                  )}
                </td>
                <td className="px-4 py-4 text-muted-foreground capitalize">
                  {r.secondReview.replace("_", " ")}
                </td>
              </tr>
            ))}
          </tbody>
