  ...
  head: () => ({
    meta: [
      { title: "Assessment overview — ARUI Assessor" },
      {
        name: "description",
        content:
          "Profile summary, applicability decisions and review counts for an institutional assessment.",
      },
    ...
    <PageContainer width="wide">
      ...
      <section className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatTile
          label="Responses"
          value={c.responses}
          detail={`${c.notSure} not sure · ${c.naRequested} N/A requested`}
        />
        <StatTile label="Evidence items" value={c.evidence} detail="Awaiting or under review" />
        <StatTile
          label="Metrics scored"
          value={`${c.metricsScored} / ${c.metricsTotal}`}
          detail="M / I / O recorded by assessor"
        />
        <StatTile
          label="Open flags"
          value={c.openFlags}
          detail="Engine and assessor flags to resolve"
        />
      </section>
      ...
      <div className="mt-10 grid gap-8 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
        <Panel>
          <PanelHeader
            ...
            title="Context as declared by the institution"
          />
          <DefinitionList
            className="px-6"
            items={a.profileSummary.map((p) => ({ term: p.label, detail: p.value }))}
          />
        </Panel>
        <Panel>
              ...
              <li key={d.domainCode} className="flex items-start gap-4 px-6 py-3.5">
                ...
                {d.applicable === null ? (
                  ...
                  <StatusBadge tone="teal">
                    {d.accepted ? "Applicable" : "Applicable · unconfirmed"}
                  </StatusBadge>
                ) : (
                  <StatusBadge tone="amber">N/A</StatusBadge>
                )}
              </li>
            ))}
          </ul>
