...
export const confidenceTone: Record<Confidence, "rose" | "amber" | "teal"> = {
  low: "rose",
  moderate: "amber",
  high: "teal",
};
...
export function MaturityScale({
  ...
  return (
    <div
      ...
      aria-label={`Current ${maturityLabels[current]}${required !== undefined ? `, required ${maturityLabels[required]}` : ""}`}
    >
      {STEPS.map((b) => {
        ...
        return (
          <span
            ...
            className={cn(
              ...
              isRequired &&
                (inverse
                  ? "ring-1 ring-inset ring-primary-foreground/80"
          ...
          >
            {isRequired && (
              <span
                className={cn(
                  "absolute -top-1.5 left-1/2 size-0 -translate-x-1/2 border-x-[4px] border-b-[5px] border-x-transparent",
                  inverse ? "border-b-primary-foreground" : "border-b-blue",
                )}
                aria-hidden
              />
            )}
...
export function ScaleLegend({ className }: { className?: string }) {
  return (
    ...
    >
      ...
      <span className="inline-flex items-center gap-2">
        <span className="h-2 w-5 rounded-[2px] bg-blue/20 ring-1 ring-inset ring-blue" /> Required
        maturity for this context
      </span>
      <span className="inline-flex items-center gap-2">
...
export function DomainPositionRow({
  domain,
  showConfidence = true,
}: {
  domain: DomainResult;
    ...
    <div className="grid items-center gap-x-5 gap-y-2 py-3.5 md:grid-cols-[3.25rem_minmax(0,14rem)_1fr_minmax(0,9rem)_minmax(0,10rem)]">
      ...
      <span className="text-xs text-muted-foreground">
        ...
        <span className="ml-1.5 text-foreground/70">
          {distance === 0 ? "at required" : `${distance} level${distance > 1 ? "s" : ""}`}
        </span>
      </span>
      {showConfidence && (
...
export function StatTile({
  ...
  return (
    <div className={cn("rounded-lg border border-border bg-card px-5 py-4 shadow-card", className)}>
      ...
      <p className="mt-2 font-serif text-2xl leading-none text-foreground md:text-[1.75rem]">
        {value}
      </p>
