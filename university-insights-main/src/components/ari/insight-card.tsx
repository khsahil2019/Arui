...
const styles: Record<InsightKind, { icon: typeof Sparkles; accent: string; label: string }> = {
  early: {
    ...
    label: "Early signal — not your final assessment",
  },
...
export function InsightCard({
  ...
  return (
    <article
      className={cn(
        "rounded-lg border border-border border-l-[3px] bg-card shadow-card",
        s.accent,
    ...
    >
      ...
      {title && (
        <h3
          className={cn(
            "mt-2 font-sans font-semibold tracking-normal text-foreground",
